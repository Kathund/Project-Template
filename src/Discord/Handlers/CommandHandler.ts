  ...
  async onCommand(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      await interaction.deferReply({
        flags: command.response === CommandResponse.Ephemeral ? MessageFlags.Ephemeral : undefined
      });
      console.discord(
  ...
  async deployCommands(): Promise<void> {
    ...
    for (const file of commandFiles) {
      ...
      if (command.data.name) {
        command.setType(CommandType[file.split('/')[0] as keyof typeof CommandType] ?? CommandType.General);
